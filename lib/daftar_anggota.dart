...
class _HalamanDaftarAnggotaState extends State<HalamanDaftarAnggota> {
  ...
  // --- FUNGSI EDIT ANGGOTA (BARU DIAKTIFKAN) ---
  Future<void> _editAnggota(Map<String, dynamic> memberData) async {
    final refresh = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => HalamanTambahAnggota(initialData: memberData),
      ),
    );

    // Otomatis merefresh daftar jika tombol simpan di halaman edit ditekan
    if (refresh == true) {
      _fetchMembers(); 
    }
  }
                      ...
                      Row(
                        ...
                        children: [
                          IconButton(
                            ...
                            onPressed: () => _editAnggota(member), // Menggunakan fungsi _editAnggota
                          ),
                          IconButton(
